...
function setTheme(dark) { document.body.classList.toggle('dark-mode', dark); const toggle = $('theme-toggle'); toggle.textContent = dark ? '☀' : '☼'; toggle.setAttribute('aria-label', dark ? 'Switch to light mode' : 'Switch to dark mode'); toggle.title = dark ? 'Switch to light mode' : 'Switch to dark mode'; toggle.setAttribute('aria-pressed', dark); localStorage.setItem('map-of-my-life-theme', dark ? 'dark' : 'light'); }
...
async function init() {
  ...
  const dataUrl = location.hostname.endsWith('.web.app') ? '/photos.json' : '/api/photos';
  try { const response = await fetch(dataUrl); if (!response.ok) throw new Error(`Photo data request failed: ${response.status}`); state.photos = (await response.json()).photos; } catch (error) { showToast('Could not load photo data.'); }
